...
public class ModuleButton {
	...
	public ModuleButton(Module imod, Panel pl) {
		 ...
		 */
		if (Ambien.INSTANCE.settingsManager.getSettingsByMod(imod) != null)
			for (Setting s : Ambien.INSTANCE.settingsManager.getSettingsByMod(imod)) {
				if (s.isCheck()) {
	...
	public boolean mouseClicked(int mouseX, int mouseY, int mouseButton) {
		...
		if (mouseButton == 0) {
			...
			if(Ambien.INSTANCE.settingsManager.getSettingByName("Sound").getValBoolean())
			...
			if (menuelements != null && menuelements.size() > 0) {
				...
				Ambien.INSTANCE.heroCodeGui.closeAllSettings();
				this.extended = b;
