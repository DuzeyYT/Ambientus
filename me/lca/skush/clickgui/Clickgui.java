package me.lca.skush.clickgui;

import me.lca.skush.Ambien;
import me.lca.skush.clickgui.comp.CheckBox;
import me.lca.skush.clickgui.comp.Combo;
import me.lca.skush.clickgui.comp.Comp;
import me.lca.skush.clickgui.comp.Slider;
...
public class Clickgui extends GuiScreen {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        offset = 0;
        for (Module m : Ambien.INSTANCE.getModuleManager().getModules(selectedCategory)) {
        ...
        for (Module m : Ambien.INSTANCE.getModuleManager().getModules(selectedCategory)) {
            if (isInside(mouseX, mouseY,posX + 65,posY + 1 + offset,posX + 125,posY + 15 + offset)) {
                ...
                if (mouseButton == 1) {
                    int sOffset = 3;
                    comps.clear();
                    if (Ambien.INSTANCE.getSettingsManager().getSettingsByMod(m) != null)
                    for (Setting setting : Ambien.INSTANCE.getSettingsManager().getSettingsByMod(m)) {
                        selectedModule = m;
                        if (setting.isCombo()) {
